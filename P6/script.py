...
data_train = descomposicion_TM[:int(0.8 * (len(descomposicion_P)))]
data_test =descomposicion_TM[int(0.8 * (len(descomposicion_P))):]
...
metrica, predicciones = backtesting_forecaster(
    forecaster= forecaster, y = descomposicion_TM["tm"].ffill(), initial_train_size = len(data_train), 
    fixed_train_size = False , steps = 20, metric = 'mean_squared_error', refit = True, verbose = True)

fig,ax = plt.subplots()
descomposicion_P.plot(label = 'test', ax=ax)
predicciones.plot(label='prediccion', ax=ax)
ax.legend();